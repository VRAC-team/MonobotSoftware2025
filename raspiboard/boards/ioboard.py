...
class IOBoard:
    ...
    def home(self, stepper_id: int, max_relative_steps_before_error: int, tor_id: int, tor_state_to_end_homing: bool):
        if stepper_id >= 5:
            return
    ...
    def goto_abs(self, stepper_id: int, absolute_steps: int, acceleleration: int, max_velocity: int):
        if stepper_id >= 5:
            return
