...
class ServoBoard:
    ...
    def servo_write_us(self, servo_id: int, servo_us: int) -> bool:
        if servo_id not in self.servos_configs:
            print(f"servo_write_us: servo_id:{servo_id} not mapped in self.servos_configs")
            return False
        
        servo_config = self.servos_configs[servo_id]

        if servo_us < servo_config.min_us or servo_us > servo_config.max_us:
            print(f"servo_write_us: invalid servo_us:{servo_us} (servo_config:{servo_config})")
            return False
    ...
    def servo_write_angle(self, servo_id: int, servo_angle: int) -> bool:
        if servo_id not in self.servos_configs:
            print(f"servo_write_angle: servo_id:{servo_id} not mapped in self.servos_configs")
            return False
        
        servo_config = self.servos_configs[servo_id]

        if servo_angle < 0 or servo_angle > servo_config.max_angle:
            print(f"servo_write_angle: invalid angle:{servo_angle} (servo_config:{servo_config})")
            return False
        
        servo_us = map_range(servo_angle, 0, servo_config.max_angle, servo_config.min_us, servo_config.max_us)
